...
interface Commander {
    fun getDispatcher(traceId: Uuid): ActionDispatcher

    fun tracingDispatcher() = getDispatcher(uuid4())
    suspend fun <T> runQuery(dispatch: suspend ActionDispatcher.() -> T): T = tracingDispatcher().dispatch()

}

object MasterCommander : Commander {
    override fun getDispatcher(traceId: Uuid): ActionDispatcher = ActionDispatcher(traceId)
}