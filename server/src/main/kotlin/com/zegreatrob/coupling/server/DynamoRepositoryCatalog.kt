...
class MemoryRepositoryCatalog private constructor(
    ...
    companion object {
        suspend operator fun invoke(userEmail: String, clock: TimeProvider): MemoryRepositoryCatalog {
            val tribeRepository = MemoryTribeRepository(userEmail, clock)
            val playerRepository = MemoryPlayerRepository(userEmail, clock)
            val pairAssignmentDocumentRepository = MemoryPairAssignmentDocumentRepository(userEmail, clock)
            val pinRepository = MemoryPinRepository(userEmail, clock)
            val userRepository = MemoryUserRepository(userEmail, clock)
