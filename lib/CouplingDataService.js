...
var CouplingDataService = function (mongoUrl) {
    ...
    this.removePlayer = function (playerId, done) {
        playersCollection.updateById(playerId, {isDeleted: true}, function () {
            done();
        });
    };

    this.removePairAssignments = function (pairAssignmentsId, done) {
        historyCollection.updateById(pairAssignmentsId, {isDeleted: true}, function (error, modifiedRecordCount) {
            if (modifiedRecordCount == 0 && error == null) {
                error = {message: 'Pair Assignments could not be deleted because they do not exist.'};
            }
            done(error);
        });
    }
