...
interface LoggingCommandExecuteSyntax : CommandExecuteSyntax, ActionLoggingSyntax {
    ...
    private suspend fun <C : SuspendResultAction<D, R>, D, R> D.handledExecute(command: C) = handleException(command) {
        command.execute(this)
    }

    private inline fun <C : SuspendResultAction<D, R>, D, R> handleException(command: C, doIt: () -> Result<R>) = try {
        doIt()
    } catch (bad: Throwable) {
