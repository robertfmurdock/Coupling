...
interface LoggingActionExecuteSyntax : ExecutableActionExecuteSyntax,
    ...
    override fun <D, R> D.execute(action: ExecutableAction<D, R>): R = action.execute(this)

    override suspend fun <D, R> D.execute(action: SuspendAction<D, R>) = handledExecute(action)
