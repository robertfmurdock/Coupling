...
class ReconcileCommand : SuspendingCliktCommand("reconcile") {
    ...
    override suspend fun run() {
        val result = TestLogTools.run(TestLogRequest(TestLogCommand.RECONCILE, listOf(path)))
        OutputRenderer("reconcile", OutputOptions(reportFile, quietSuccess, failureSummary)).render(result)
    }
}
