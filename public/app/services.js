...
services.service("Coupling", ['$http', '$resource', '$q', function ($http, $resource, $q) {
    ...
    var post = function (url, player) {
        return $http.post(url, player).then(function (result) {
            return result.data;
    ...
    this.getTribes = function () {
        var url = '/api/tribes';
        return Tribe.query().$promise.catch(function (response) {
    ...
    this.requestPlayersPromise = function (tribeId, historyPromise) {
        ...
        return $q.all({
            players: $http.get(url).then(function (response) {
                return response.data;
            }, function (response) {
                var data = response.data;
                var statusCode = response.status;
    ...
    this.spin = function (players, tribeId) {
        ...
        return $http.post(url, players).then(function (result) {
            return result.data;
        }, logAndRejectError('POST ' + url));
    };
    this.saveCurrentPairAssignments = function (tribeId, pairAssignments) {
        var url = '/api/' + tribeId + '/history';
        return $http.post(url, pairAssignments).then(function (result) {
            return result.data;
        }, logAndRejectError('POST ' + url));
    };
